...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    #
	# construct A and b
    A = matrix() # construct the jacobian matrix 
    num_pose = len(data) + 1 # number of poses (motion odometry + 1)
    dim = num_landmarks + num_pose # number of poses plus number of landmarks 
    A.zero(dim*2, dim*2) # x0 x1 ... xn l0 l1 ... lm, *2 since each pose and landmark has (x,y) 
    b = matrix() # construct residual vector 
    b.zero(dim*2, 1) 
    
    # set initial constraint 
    A.value[0][0] = 1.;
    A.value[1][1] = 1.; 
    b.value[0][0] = 50.; # set it starts at the center of the world 
    b.value[1][0] = 50.;
    
    # weight/confidence for motion and measurement 
    ...
    for i in range(len(data)):
        ...
        for landmark in Zi:
            ...
            l_dx = landmark[1]
            l_dy = landmark[2]
            # add measurement [l_id, l_dx, l_dy] into A and b
            # xi - lj = -mij
            A.value[i*2][i*2] += 1.*w_measure 
            A.value[i*2][(num_pose+l_id)*2] += -1.*w_measure
            b.value[i*2][0] += -l_dx * w_measure
            
            # lj - xi = mij
            A.value[(num_pose+l_id)*2][i*2] += -1.*w_measure 
            A.value[(num_pose+l_id)*2][(num_pose+l_id)*2] += 1.*w_measure
            b.value[(num_pose+l_id)*2][0] += l_dx * w_measure
            
            # similar operation for dy
            
            # xi - lj = -mij
            A.value[i*2+1][i*2+1] += 1.*w_measure 
            A.value[i*2+1][(num_pose+l_id)*2+1] += -1.*w_measure
            b.value[i*2+1][0] += -l_dy * w_measure
            
            # lj - xi = mij
            A.value[(num_pose+l_id)*2+1][i*2+1] += -1.*w_measure 
            A.value[(num_pose+l_id)*2+1][(num_pose+l_id)*2+1] += 1.*w_measure
            b.value[(num_pose+l_id)*2+1][0] += l_dy * w_measure
        ...
        # xi - xi+1 = -uii+1 
        A.value[i*2][i*2] += 1.*w_motion
        A.value[i*2][(i+1)*2] += -1.*w_motion
        b.value[i*2][0] += -u_dx * w_motion
        
        # xi+1 - xi = uii+1
        A.value[(i+1)*2][i*2] += -1.*w_motion
        A.value[(i+1)*2][(i+1)*2] += 1.*w_motion
        b.value[(i+1)*2][0] += u_dx * w_motion
        
        # similar operation for dy 
        # xi - xi+1 = -uii+1 
        A.value[i*2+1][i*2+1] += 1.*w_motion
        A.value[i*2+1][(i+1)*2+1] += -1.*w_motion
        b.value[i*2+1][0] += -u_dy * w_motion
        
        # xi+1 - xi = uii+1
        A.value[(i+1)*2+1][i*2+1] += -1.*w_motion
        A.value[(i+1)*2+1][(i+1)*2+1] += 1.*w_motion
        b.value[(i+1)*2+1][0] += u_dy * w_motion
    
    # OK, solve it mu = A-1 * b 
    mu = A.inverse()*b
